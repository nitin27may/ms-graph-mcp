...
| `401` on a hosted call with a token that looks right | Check `aud`, not `azp`. The default posture needs a token audienced to **this server** (`api://<client id>`), not one for Graph. See [agent-auth.md](agent-auth.md). |
